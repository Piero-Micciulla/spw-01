...
const siteUrl=String(process.env.SITE_URL||config.siteUrl).replace(/\/$/,'');
...
for(const file of ['index.html','privacy/index.html','404.html','robots.txt','sitemap.xml']){const path=join(output,file);const rendered=(await readFile(path,'utf8')).replaceAll('{{SITE_URL}}',siteUrl).replaceAll('{{CURRENT_YEAR}}',String(new Date().getFullYear()));await writeFile(path,rendered)}
...
console.log(`Built static site in ${output}`);console.log(siteUrl?`Canonical origin: ${siteUrl}`:'Canonical origin is not configured; set siteUrl before deployment.');
