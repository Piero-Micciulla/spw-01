...
const required = ['index.html','privacy/index.html','404.html','robots.txt','sitemap.xml','styles.css','runtime-config.js','js/app.js','js/catalog.js','js/picker.js','assets/icon.svg','assets/social-card.jpg','site.webmanifest'];
await Promise.all(required.map(file => access(join('dist', file))));
const html = await readFile('dist/index.html', 'utf8');
for (const [name, pattern] of [
  ['title', /<title>[^<]+<\/title>/], ['description', /<meta name="description" content="[^"]+"/],
  ['canonical', /<link rel="canonical" href="(?:https:\/\/[^"}]+|\/)"/], ['Open Graph', /property="og:title"/],
  ['Twitter card', /name="twitter:card"/], ['main heading', /<h1[ >]/], ['structured data', /application\/ld\+json/],
  ['picker control', /id="pick-button"/], ['privacy link', /href="\/privacy\/"/]
...
if (social.size < 20_000 || social.size > 300_000) throw new Error('Social image appears incomplete or insufficiently optimized');
...
console.log(`Build checks passed: required files, metadata, placeholders, integrations, and social image verified. Dist size: ${(await total('dist') / 1024).toFixed(1)} KiB.`);
