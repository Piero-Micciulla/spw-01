...
const config=JSON.parse(await readFile(join(root,'site.config.json'),'utf8')),siteUrl=String(process.env.SITE_URL||config.siteUrl).replace(/\/$/,'');
...
await writeFile(join(output,'runtime-config.js'),'window.SITE_CONFIG={};\n');console.log(`Built static site in ${output}`);
