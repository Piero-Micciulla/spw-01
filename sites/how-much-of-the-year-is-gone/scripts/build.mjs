...
for (const file of ['index.html','privacy/index.html','404.html','robots.txt','sitemap.xml']) {
  const path = join(output, file);
  const rendered = (await readFile(path, 'utf8')).replaceAll('{{SITE_URL}}', siteUrl).replaceAll('{{CURRENT_YEAR}}', currentYear);
